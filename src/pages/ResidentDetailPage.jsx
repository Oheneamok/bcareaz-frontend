...
export default function StaffDetailPage() {
  ...
  useEffect(() => {
    ...
    if (tab) {
      const matchedTab = tabs.find((t) => t.label.toLowerCase() === tab.toLowerCase());
      if (matchedTab) setActiveTab(matchedTab.label);
    }

    if (action === "add-evidence" && task && complianceItems.length) {
      const decodedTask = decodeURIComponent(task).toLowerCase();
      const matchedItem = complianceItems.find((item) =>
        String(item.key || "").toLowerCase() === decodedTask ||
        String(item.title || "").toLowerCase() === decodedTask
      );

      if (matchedItem && !taskModalItem) {
        setActiveTab(matchedItem.source_tab || matchedItem.section || "Compliance");
        openComplianceTask(matchedItem);
      }
    }
  }, [location.search, complianceItems, taskModalItem]);
        ...
        <label className="staff-task-upload">
          ...
          <input
            ...
            onChange={(event) => {
              ...
              update("evidence_url", form.evidence_url || `/uploads/staff-compliance/${encodeURIComponent(file.name)}`);
            }}
          />
        </label>
